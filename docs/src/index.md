...
`Whitebox.jl` provides a function for each of the the tools provided by `WhiteboxTools`. Functions in `Whitebox.jl` use the snake_case convention for `WhiteboxTools` tool names. 
